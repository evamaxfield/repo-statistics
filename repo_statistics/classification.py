...
def get_heuristic_project_type(
    ...
    # Compute ratio
    try:
        ratio_stargazers_to_contribs = stargazers_count / total_contributors_count
    except ZeroDivisionError:
        return "undefined"

    # Categorize
    if total_contributors_count <= 5 and ratio_stargazers_to_contribs < 5:
        return "single-experiment"  # toy

    if total_contributors_count > 5 and ratio_stargazers_to_contribs < 5:
        return "lab-level-tool"  # club

    if total_contributors_count > 20 and ratio_stargazers_to_contribs >= 5:
