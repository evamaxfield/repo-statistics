...
| [AI Code Detection Metrics](#16-ai-code-detection-metrics) | `ai_detection_functions_sampled_count`, `ai_detection_files_sampled_count`, `ai_detection_{dataset}_{model}_ai_function_proportion`, `ai_detection_{dataset}_{model}_ai_confidence_mean`, `ai_detection_cache_hits` | AI authorship classification scores over a fixed-budget stratified random sample of functions/files per repo-year, with cross-call exact-text caching, measured at the analysis end datetime |
